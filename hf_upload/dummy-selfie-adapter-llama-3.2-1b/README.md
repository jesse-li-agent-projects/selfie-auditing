...
publishes this fixture. `init_scale = 0.9332588315010071`, also recorded in this
checkpoint's safetensors metadata header.

Verify by content with a tolerance, not by hash or exact equality.

Regenerating at seed 0 reproduces the tensors, but never the file bytes:
safetensors serializes the `__metadata__` map in a per-process order, so two
runs on one machine already differ by hash while holding identical tensors and
identical metadata values.

`init_scale` is the one value that depends on the device it was generated on.
It is a median over the base model's embedding rows, and that reduction differs
in its last bits (`0.9332589507102966` on CUDA). Each value is stable on its own
device. `bias` is scaled by `init_scale`, so it shifts with it. The relative
difference is ~1e-7 and irrelevant to a fixture whose purpose is shape
checking — read the value from the header rather than recomputing it.
